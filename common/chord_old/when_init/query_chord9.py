...
from common.table_class.chord_old import Table_Chord9
...
def execute(root_note_instance, chord_term):
    ...
    note9_meta = aliased(Table_Meta)
    # aaa = aliased(Table_Meta)
    data = session.query(Table_Chord9, note3_meta, note5_meta, note7_meta,note9_meta).outerjoin(
        note3_meta, note3_meta.uid == Table_Chord9.n3_uid).outerjoin(
        note5_meta, note5_meta.uid == Table_Chord9.n5_uid).outerjoin(
        note7_meta, note7_meta.uid == Table_Chord9.n7_uid).outerjoin(
        note9_meta, note9_meta.uid == Table_Chord9.n9_uid).filter(
        Table_Chord9.chord_term == chord_term).filter(
        Table_Chord9.rn_uid == root_note_instance.uid).filter(
        Table_Chord9.is_valid == 1).one_or_none()
    if data:
        ...
        note_9_uid = getattr(data[4], "uid")
        note_9_octave = getattr(data[0], "n9_octave")
        note_9_instance = Note(note_9_uid, root_note_instance.octave + note_9_octave)
        return [root_note_instance, note_3_instance, note_5_instance, note_7_instance, note_9_instance]
    return None
