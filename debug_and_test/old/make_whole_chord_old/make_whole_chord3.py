...
from common.table_class.chord_old import Table_Chord3, Table_Whole_Chord
from sqlalchemy.orm import sessionmaker
from debug_and_test.old.make_whole_chord_old import the_function


def make_whole_chord3():
    print("开始处理 「 三和弦 」 ... ")
    ...
    adjust_list = [None, "add2", "sus2", "sus4", "add4", "add6"]
    for i in res:
        ...
        for adjust in adjust_list:
            ...
            if adjust == "add2":
                ...
                adjusted_list = the_function.add(origin_list, 9)
                the_function.insert_to_whole_chord(root_note_uid=rn_uid, rn_math_name=rn_math_name,
                                                   base_chord=chord_term, inset_list=adjusted_list, is_add9=True)
            elif adjust == "add11":
                adjusted_list = the_function.add(origin_list, 11)
                the_function.insert_to_whole_chord(root_note_uid=rn_uid, rn_math_name=rn_math_name,
                                                   base_chord=chord_term, inset_list=adjusted_list, is_add11=True)
            elif adjust == "add13":
                adjusted_list = the_function.add(origin_list, 13)
                the_function.insert_to_whole_chord(root_note_uid=rn_uid, rn_math_name=rn_math_name,
                                                   base_chord=chord_term, inset_list=adjusted_list, is_add13=True)
    print("「 三和弦 」全部处理完毕...")
