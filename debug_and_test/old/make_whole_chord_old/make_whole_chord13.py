...
from common.table_class.chord_old import Table_Chord13, Table_Whole_Chord
from sqlalchemy.orm import sessionmaker
from debug_and_test.old.make_whole_chord_old import the_function


def make_whole_chord13():
    print("开始处理 「 十三和弦 」 ... ")
    from note import Note
    engine = create_engine(DB_SQL_LOCATION)
    session = sessionmaker(bind=engine)()

    res = session.query(Table_Chord13).all()
    adjust_list = [None, "sus2", "sus4",  "sharp5", "flat5", "omit3", "omit5", "omit7", "omit9",
                   "sus2_omit5", "omit11",
                   "sus2_omit7", "sus2_omit9",
                   "sus4_omit5", "sus4_omit7", "sus2_add4", "sus2_add6", "sus4_add6", "sus2_add4_omit5",
                   "sus2_add4_omit7"]
    for i in res:
        ...
        origin_list = [
            ...
            Note(getattr(i, "n11_uid"), CENTER_C_LOCATION + int(getattr(i, "n11_octave"))),
            Note(getattr(i, "n13_uid"), CENTER_C_LOCATION + int(getattr(i, "n13_octave"))),
        ]
        for adjust in adjust_list:
            ...
            if adjust == "sus2":
                ...
                adjusted_list = the_function.omit(origin_list, 9)
                the_function.insert_to_whole_chord(root_note_uid=rn_uid, rn_math_name=rn_math_name,
                                                   base_chord=chord_term, inset_list=adjusted_list, is_omit9=True)

            elif adjust == "omit11":
                adjusted_list = the_function.omit(origin_list, 9)
                the_function.insert_to_whole_chord(root_note_uid=rn_uid, rn_math_name=rn_math_name,
                                                   base_chord=chord_term, inset_list=adjusted_list, is_omit11=True)
